...
1. **Yêu cầu:** Python 3.10+, Redis Server (để chạy Celery).
2. **Cài đặt thư viện:**
   ```bash
...
5. **Khởi chạy Server:**
   ...
   ```
6. **Khởi chạy Worker (OCR):**
   ```bash
   celery -A abc_core worker -l info
   ```

## 📈 Lộ trình Phát triển (Roadmap)
- [ ] Tích hợp Elasticsearch cho Full-text search quy mô lớn.
- [ ] Xây dựng App Mobile (React Native) để duyệt công văn trên điện thoại.
- [ ] Tích hợp Watermark tự động cho file tải xuống để chống rò rỉ dữ liệu.
